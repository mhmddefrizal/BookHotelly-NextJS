  ...
  return (
    <div>
      <form action={formAction}>
        ...
        <button
          type="submit"
          className={clsx("w-full bg-blue-600 text-white py-3 rounded-sm font-medium hover:bg-blue-700 transition-colors", isPending && "opacity-50 cursor-not-allowed")}
          // pasang atribut kontrol untuk menonaktifkan tombol saat pengiriman form sedang berlangsung
          disabled={isPending}
        >
          {isPending ? "Loading..." : "Memesan"}
        </button>
      </form>
    </div>
  );
